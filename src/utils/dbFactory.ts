...
import { GokuComment, GokuUser, TestComment, TestUser, TestTransaction } from "../models/index";
...
const DbFactory = {
	...
	createInstance: (appMode: APP_ENV | string | undefined): void | IDbCommunication => {
		if (appMode === "local") {
			...
			instance = new DbClient("production", GokuUser, GokuComment, TestTransaction);
		} else {
